          ...
          for (const item of localList) {
            if (!item._synced) {
              try {
                ...
                const selectedTimeSlot = item.timeSlot || item.time_slot || item.timeslot || item.slot || item.time || '09:00 AM - 11:00 AM';
                const selectedBikeModel = item.bikeModel || item.bike_model || item.bikemodel || item.bike || item.vehicle || item.vehicleModel || item.model || 'Bike Service';
                ...
                const payload = {
                  ...
                  'Vehicle Model': selectedBikeModel,
                  'Bike Model': selectedBikeModel,
                  serviceName: item.serviceName || item.service || 'Complete Service',
                  plan: item.planName || item.plan || 'Premium Care',
                  planName: item.planName || item.plan || 'Premium Care',
                  'Plan': item.planName || item.plan || 'Premium Care',
